...
import { useRoqPlatformUserProfiles, useProductCount, useInstallationCount, useOrderCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const installationStat = useInstallationCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Installations',
      result: installationStat?.data,
      isLoading: installationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Orders',
